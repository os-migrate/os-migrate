...
class AnsibleAutoPluginDirective(Directive):
    ...
    def make_node(self, title, contents, content_type=None):
        ...
        if not content_type:
            ...
            authors_list.append(self._raw_fields(data=contents["author"]))
            ...
            for content in contents:
                section.append(self._literal_block(data=content, dump_data=False))
        return section

    @staticmethod
    def load_module(filename):
        """Load the module."""
        return load_source("__ansible_module__", filename)
    ...
    def _literal_block(data, language="yaml", dump_data=True):
        if dump_data:
            literal = nodes.literal_block(text=DOCYAML.dump(data))
        else:
    ...
    def _yaml_section(self, to_yaml_data, section_title, section_text=None):
        yaml_section = self._section_block(title=section_title, text=section_text)
    ...
    def _run_role(self, role):
        ...
        section = self._section_block(
            title="Role Documentation", raw_html=True, text=html
        )
        ...
        if os.path.exists(molecule_path):
            for test in os.listdir(molecule_path):
                ...
                molecule_playbook_path = os.path.join(
                    molecule_path, test, "converge.yml"
                )
    ...
    def _run_module(
        self, module, module_title="Module Documentation", example_title="Example Tasks"
    ):
        if self.options.get("documentation"):
            docs = self.build_documentation(module=module)
            self.run_returns.append(self.make_node(title=module_title, contents=docs))

        if self.options.get("examples"):
            examples = self.build_examples(module=module)
            self.run_returns.append(
                self.make_node(
                    title=example_title, contents=examples, content_type="yaml"
                )
            )
    ...
    def run(self):
        ...
        if self.options.get("module"):
            module_path = os.path.join(
                os.environ["PROJECT_ROOT"], self.options["module"]
            )
