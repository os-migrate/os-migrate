...
class UserProjectRoleAssignment(resource.Resource):
    ...
    def _refs_from_sdk(conn, sdk_res):
        ...
        refs['role_ref'] = reference.role_ref(
            conn, sdk_res['role']['id'])

        refs['user_id'] = sdk_res['user']['id']
        refs['user_ref'] = reference.user_ref(
            conn, sdk_res['user']['id'])
    ...
    def _refs_from_ser(self, conn):
        ...
        refs['project_id'] = reference.project_id(
            conn, self.params()['project_ref'])

        refs['role_ref'] = self.params()['role_ref']
        refs['role_id'] = reference.role_id(
            conn, self.params()['role_ref'])
    ...
    def create_or_update(self, conn, filters=None):
        ...
        assignments = conn.identity.role_assignments(**sdk_params)
        arr = []
    ...
    def _is_same_resource(self, target_data):
        ...
        same_project = (self.data[const.RES_INFO].get('project_id', '__undefined1__') ==
                        target_data[const.RES_INFO].get('project_id', '__undefined2__'))
        same_role = (self.data[const.RES_INFO].get('role_id', '__undefined1__') ==
                     target_data[const.RES_INFO].get('role_id', '__undefined2__'))
        same_user = (self.data[const.RES_INFO].get('user_id', '__undefined1__') ==
