...
from ansible_collections.os_migrate.os_migrate.plugins.module_utils import resource
...
class TestSerialization(unittest.TestCase):
    ...
    def test_add_or_replace_resource(self):
        ...
        # append at the end
        self.assertTrue(serialization.add_or_replace_resource(resources, add_resource))
        self.assertEqual(
            resources,
            [
                {
        ...
        # replace existing
        self.assertTrue(
            serialization.add_or_replace_resource(resources, replace_resource)
        )
        self.assertEqual(
            resources,
            [
    ...
    def test_resource_needs_update_nested(self):
        ...
        target[const.RES_PARAMS]["subresources"][0][const.RES_INFO][
            "nested-detail"
        ] = "updated detail"
        self.assertFalse(serialization.resource_needs_update(current, target))

        target[const.RES_PARAMS]["subresources"][0][const.RES_PARAMS][
            "name"
        ] = "updated-name"
        self.assertTrue(serialization.resource_needs_update(current, target))
        ...
        target[const.RES_PARAMS]["subresources"][0][
            const.RES_TYPE
    ...
    def test_set_sdk_params_same_name(self):
        ser_params = {"a": "b", "c": "d", "e": "f"}
        sdk_params = {"g": "h"}
        serialization.set_sdk_params_same_name(ser_params, sdk_params, ["a", "e", "z"])
        self.assertEqual(sdk_params, {"a": "b", "g": "h", "e": "f"})

    def test_set_ser_params_same_name(self):
        ser_params = {"g": "h"}
        sdk_params = {"a": "b", "c": "d", "e": "f"}
        serialization.set_ser_params_same_name(ser_params, sdk_params, ["a", "e"])
